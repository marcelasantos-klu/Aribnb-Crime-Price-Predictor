...
def main(split_strategy: Optional[str] = None, prompt_for_split: bool = False) -> None:
    ...
    with prior outlier removal on the target.

    Steps performed:
    1. Load the dataset and basic configuration.
    2. Remove outliers from the target using an IQR rule (default k=1.5).
    3. Build a shared preprocessing pipeline for numeric and categorical features.
    4. Split the data into train and test sets, stratified by city (configurable).
    ...
    5. For each model type (Linear Regression, Decision Tree, Random Forest,
    ...
    6. After all models are trained, aggregate the metrics into a summary table,
